...
//    }
    ...
    @POST
    @Produces(value = {MediaType.APPLICATION_JSON})
    public Response loginSignUp(@PathParam("email") final String email,
                                @Valid NewUser newUserDto,
                                final BindingResult errors) {

        if(!Objects.equals(newUserDto.getPassword(), newUserDto.getConfirmPassword())) {
            errors.rejectValue("confirmPassword","EditProfile.NotSamePassword");
//            TODO: VER ESTO QUE RETORNAR Y ESO O SI SACARLO
//            return LoginSingUp(userDetails, loginForm, loginStage);
        }

        User user = us.register(newUserDto.getEmail(), newUserDto.getUsername(), newUserDto.getPassword()).get();
        LOGGER.info("POST /{}: Registered a new user with email", uriInfo.getPath());
//        TODO: VER ESTO DEL AUTH
//        us.authWithAuthManager(request, loginForm.getEmail(), loginForm.getPassword(), authenticationManager);

        LOGGER.info("POST /{}: User Creation", uriInfo.getPath());

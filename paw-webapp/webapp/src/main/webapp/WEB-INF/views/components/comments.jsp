                    ...
                    <c:choose>
                        <c:when test="${param.loggedUserName != 'null'}">
                            <c:if test="${comment.user.userName == param.loggedUserName || param.isAdmin == 'true'}">
                                <form class="W-delete-form" id="<c:out value="formDeleteComment${comment.commentId}"/>" method="post" action="<c:url value="/comment/${comment.commentId}/delete"/>">
                                    <button class="btn btn-danger text-nowrap W-prueba"  type="button" data-bs-toggle="modal" data-bs-target="<c:out value=" #deleteCommentModal${comment.commentId}"/>" >
                                ...
                                <div class="modal fade" id="<c:out value="deleteCommentModal${comment.commentId}"/>" tabindex="-1" aria-labelledby="<c:out value="modalLabelDeleteComment${comment.commentId}"/>" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button type="submit" form="<c:out value="formDeleteComment${comment.commentId}"/>" class="btn btn-success" onclick="this.form.submit(); (this).className += ' spinner-border'; (this).innerText = '|'"><spring:message code="Yes"/></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <c:if test="${comment.user.userName != param.loggedUserName && !param.alreadyReport}">
                                ...
                                <span title="${reportCommentAdd}">
                                    <button id="reportCommentButton" type="button" class="btn btn-light W-background-color-report" data-bs-toggle="modal" data-bs-target="<c:out value="#reportCommentModal${comment.commentId}"/> ">
                                ...
                                <div class="modal fade" id="<c:out value="reportCommentModal${comment.commentId}"/>" tabindex="-1" aria-labelledby="<c:out value="reportCommentModalLabel${comment.commentId}"/>" aria-hidden="true">
                                    <div class="modal-dialog">
                                        ...
                                        <form:form id="reportCommentForm" modelAttribute="reportCommentForm" action="${postPath}" method="post" enctype="multipart/form-data">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="<c:out value="reportCommentModalLabel${comment.commentId}"/>"><spring:message code="Report.CommentTitle"/></h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <div>

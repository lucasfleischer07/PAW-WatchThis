...
<div class="accordion W-accordion-margin" id="<c:out value="accordion${param.reviewId}"/>">
                        ...
                        <c:choose>
                            <c:when test="${param.userName.equals(param.loggedUserName)}">
                                <div>
                                    <div class="W-delete-edit-buttons">
                                        <form  class="W-delete-form" id="<c:out value="formDeleteReview${param.reviewId}"/>" method="post" action="<c:url value="/review/${param.reviewId}/delete"/>">
                                            <button id="<c:out value= "deleteReviewButton${param.reviewId}"/>" class="btn btn-danger text-nowrap"  type="button" data-bs-toggle="modal" data-bs-target="#<c:out value="modalDeleteReview${param.reviewId}"/>">
                            ...
                            <c:when test="${param.isAdmin}">
                                <div>
                                    <div class="W-delete-edit-buttons">
                                        <form  class="W-delete-form" id="<c:out value="formDeleteReview${param.reviewId}"/>" method="post" action="<c:url value="/review/${param.reviewId}/delete"/>">
                                            <button class="btn btn-danger text-nowrap"  type="button" data-bs-toggle="modal" data-bs-target="#<c:out value="modalDeleteReview${param.reviewId}"/>">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
                                                    <path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5ZM11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H2.506a.58.58 0 0 0-.01 0H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1h-.995a.59.59 0 0 0-.01 0H11Zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5h9.916Zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47ZM8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5Z"/>
                                                </svg>
                                            </button>
                            ...
                            <c:choose>
                                <c:when test="${param.loggedUserName != 'null'}">
                                    <c:if test="${(!param.alreadyReport) && (param.userName != param.loggedUserName)}">
                                        ...
                                        <button id="<c:out value="reportReviewButton${param.reviewId}"/>" type="button" class="btn btn-light W-background-color-report" data-bs-toggle="modal" data-bs-target="#<c:out value="reportReviewModal${param.reviewId}"/>">
                                ...
                                <c:otherwise>
                                    <c:if test="${param.userName != param.loggedUserName}">
                                        <spring:message code="Report.Review.Add" var="reportReviewAdd"/>
                                        <span title="${reportReviewAdd}">
                                            <button id="<c:out value="reportReviewButton${param.reviewId}"/>" type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#<c:out value="reportReviewModalNoLogin${param.reviewId}"/>">
    ...
    <div class="modal fade" id="<c:out value="modalDeleteReview${param.reviewId}"/>" tabindex="-1" aria-labelledby="<c:out value="modalLabel${param.reviewId}"/>" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" form="<c:out value="formDeleteReview${param.reviewId}"/>" class="btn btn-success" onclick="this.form.submit(); (this).className += ' spinner-border'; (this).innerText = '|'"><spring:message code="Yes"/></button>
                </div>
            </div>
        </div>
    </div>
    <div class="modal fade" id="<c:out value="reportReviewModal${param.reviewId}"/>" tabindex="-1" aria-labelledby="<c:out value="reportReviewModalLabel${param.reviewId}"/>" aria-hidden="true">
        <div class="modal-dialog">
            ...
            <form:form id="reportReviewForm${param.reviewId}" modelAttribute="reportReviewForm" action="${postPath}" method="post" enctype="multipart/form-data">
                <div class="modal-content">
                    <div class="modal-header">
